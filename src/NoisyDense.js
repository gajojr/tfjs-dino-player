...
class NoisyDense extends tf.layers.Layer {
	...
	build(inputShape) {
		...
		this.kernelStddev = this.addWeight(
			'kernelStddev',
			[inputShape[1], this.units],
			'float32',
			tf.initializers.constant({
				value: this.sigma / Math.sqrt(inputShape[1]),
			}),
			null,
			false
		);
		if (this.useBias) {
			this.biasStddev = this.addWeight(
				'biasStddev',
				[this.units],
				'float32',
				tf.initializers.constant({ value: this.sigma }),
				null,
				false
			);
		}
	...
	call(inputs) {
		...
		const stddev = this.useFactorised
			? tf.outer(tf.ones(noiseShape), kernelStddev)
			: kernelStddev;
		...
		if (this.useBias) {
			...
			const biasStddevs = this.useFactorised
				? tf.outer(tf.ones(biasNoiseShape), biasStddev)
				: biasStddev;
